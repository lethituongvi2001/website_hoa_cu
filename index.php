...
switch ($action) {
    ...
    case "luudonhang":
        ...
        $khachhang_id = $kh->laythongtinkhachhang($email);
        var_dump($khachhang_id["id"]);
        // lưu địa chỉ khách
        $dc = new DIACHI();
        $diachi_id = $dc->themdiachi($khachhang_id["id"], $diachi);

        // lưu đơn hàng
        $dh = new DONHANG();
        $tongtien = tinhtiengiohang();
        $donhang_id = $dh->themdonhang($khachhang_id["id"], $diachi_id, $tongtien);
