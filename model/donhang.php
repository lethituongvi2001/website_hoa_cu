...
class DONHANG
{
	...
	public function doctatcadonhang()
	{
		$db = DATABASE::connect();
		try {
			$sql = "SELECT dh.*, nd.*, dc.* FROM nguoidung nd, donhang dh, diachi dc WHERE nd.id = dh.nguoidung_id and dc.id = dh.diachi_id ORDER BY dh.id DESC";
			$cmd = $db->prepare($sql);
			// $cmd->bindValue(":id", $nguoidung_id);
			$cmd->execute();
			$result = $cmd->fetchAll();
	...
	public function doctatcadonhangchitiet($id)
	{
		$db = DATABASE::connect();
		try {
			$sql = "SELECT * FROM nguoidung nd, donhang dh, donhangct ct, diachi dc WHERE nd.id = dh.nguoidung_id and ct.donhang_id = dh.id and dc.id = dh.diachi_id and ct.donhang_id=:id";
			$cmd = $db->prepare($sql);
			$cmd->bindValue(":id", $id);
			$cmd->execute();
			$result = $cmd->fetchAll();
